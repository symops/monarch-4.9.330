...
blk_qc_t submit_bio(struct bio *bio)
{
	...
	if (bio_has_data(bio)) {
		...
		if (unlikely(block_dump)) {
			...
			if (block_dump_count > 0 && strncmp(_comm, current->comm, sizeof(_comm))!=0 && strncmp(bdevname(bio->bi_bdev, b), "sd", 2)==0 && strncmp(current->comm, "jbd2", 4)!=0 && strncmp(current->comm, "kworker", 7)!=0 ) {
				block_dump_count--;
