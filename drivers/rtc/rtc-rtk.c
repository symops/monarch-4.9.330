...
static int rtk_rtc_gettime_virtual(struct device *dev, struct timespec *ts)
{
	struct mtd_info *mtd;
	int err,retlen;
	u32 wd_time_seconds = 0, wd_time_xor_seconds = 0;
    u32 buf[2] = {0};

	mtd = get_mtd_device(NULL, 0);
	if (IS_ERR(mtd)) {
		err = PTR_ERR(mtd);
		dev_err(dev, "Cannot get MTD device\n");
		return err;
	}
	dev_info(dev,"rtc open mtd success\n");
	
    /* read 2 bytes from spi flash , If read > 2 bytes , system will kernel panic */
    mtd_read(mtd, 0, sizeof(buf), (size_t *)&retlen, (unsigned char *)buf);
    wd_time_seconds = buf[0];
    wd_time_xor_seconds = buf[1];
    
    //dev_info(dev, "wd_time_seconds = %x\n",wd_time_seconds);
    //dev_info(dev, "wd_time_xor_seconds = %x\n",wd_time_xor_seconds);
    ...
    if( (wd_time_seconds ^ wd_time_xor_seconds) == UINT_MAX ) {
        ...
        dev_err(dev, "Invalid time read from flash, use default time\n");
        ts->tv_sec = mktime64(2021, 1, 1, 0, 0, 0);
        ts->tv_nsec = 0;
    }
...
static int rtk_rtc_settime_virtual(struct device *dev, u32 cur_sec)
{
#ifdef CONFIG_MTD
	int err, retlen;
	struct mtd_info *mtd;
    ...
    rtk_rtc_gettime_virtual(dev,&ts);
	if( cur_sec - ts.tv_sec < FLASH_WRITE_LIMIT ) {
		dev_info(dev, "Ignore set time to flash during %d seconds\n", FLASH_WRITE_LIMIT);
		return -1;
	}
	mtd = get_mtd_device(NULL, 0);
	if (IS_ERR(mtd)) {
	...
	dev_info(dev, "Write cur_sec(%u) [0x%08X, 0x%08X] to flash\n", cur_sec, buf[0], buf[1]);
	...
	if( !mtd_erase(mtd, &ei) && !(mtd_write(mtd, 0, sizeof(buf), (size_t *)&retlen, (unsigned char *)buf))) {
		...
		dev_err(dev, "Erase/Write failed\n");
		return -1;
	}
#else
	return -1;
...
static int rtk_rtc_set_time(struct device *dev, struct rtc_time *tm)
{
    ...
    return rtk_rtc_settime_virtual(dev, cur_sec);
}
...
static int rtk_rtc_probe(struct platform_device *pdev)
{
    ...
    wait_mtd_ready();
    dev_info(dev,"rtc wait_mtd_ready finish\n");
